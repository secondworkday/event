...
app.run(['$rootScope', '$state', '$stateParams', '$http', '$templateCache', 'utilityService', 'AUTHORIZATION_ROLES', function ($rootScope, $state, $stateParams, $http, $templateCache, utilityService, AUTHORIZATION_ROLES) {
  ...
  $http.get('/client/app/public/public.html', { cache: $templateCache });
  $http.get('/client/app/public/connection/reconnecting.html', { cache: $templateCache });
  $http.get('/client/app/public/connection/disconnected.html', { cache: $templateCache });
...
app.service('utilityService', ['$rootScope', '$q', '$state', '$http', '$window', '$log', 'msIdentity', 'msAuthenticated', 'SYSTEM_INFO', 'TEMPLATE_URL', 'CONNECTION_STATUS', 'CONNECTION_EVENT', 'USER_STATE', 'BADGE_COLORS', function ($rootScope, $q, $state, $http, $window, $log, msIdentity, msAuthenticated, SYSTEM_INFO, TEMPLATE_URL, CONNECTION_STATUS, CONNECTION_EVENT, USER_STATE, BADGE_COLORS) {
  ...
  self.createModelCache = function (hubSearchHandler, unPackHandler) {
    ...
    return self.createItemCache(serviceSearchHandler, unPackHandler);
  };

  // used by *external* callers, providing a service search method - like 'siteService.searchShowNameAliases'
  self.createItemCache = function (serviceSearchHandler, unPackHandler) {
    var modelItems = {
      hashMap: {},
      index: [],
      delayLoad: {
        // keys that we're currently in-flight fetching
        activeItemKeys: [],
        // promised that is resolved when nextItemKeys are fetched
        nextDeferred: $q.defer(),
        // queue of items to be fetched from the server with our next flight
        nextItemKeys: []
      }

    };

    if (angular.isFunction(unPackHandler)) {
      modelItems.unPackHandler = unPackHandler;
    }
    ...
    // itemData can be a single itemKey (id or code), or an array of itemKeys
    modelItems.ensure = function (itemData) {
      ...
      return delayLoad(missingItemKeys)
      .then(function () {
    ...
    // itemData can be a itemKey (id or code) or array of itemKeys
    function delayLoad(itemData) {

      var delayLoadData = modelItems.delayLoad;

      // ** Queue up any work we're passed in onto our next flight ...
      if (angular.isString(itemData) || angular.isNumber(itemData)) {
        return delayLoad([itemData]);
      }
      if (itemData) {
        angular.forEach(itemData, function (itemKey) {
          if (delayLoadData.activeItemKeys.indexOf(itemKey) === -1 && delayLoadData.nextItemKeys.indexOf(itemKey) === -1) {
            // this is a new itemID - add it to our work Q ...
            delayLoadData.nextItemKeys.push(itemKey);
          }
        });
      }

      // If we have a an active flight, return a promise to get this work done by the end of our next flight
      if (delayLoadData.activeItemKeys.length > 0) {
        return delayLoadData.nextDeferred.promise;
      }

      // We're idle. See if we should launch our next flight

      if (delayLoadData.nextItemKeys.length > 0) {
        // Yes, we're going to issue a new request!

        // next becomes active ...
        var activeDeferred = delayLoadData.nextDeferred;
        delayLoadData.activeItemKeys = delayLoadData.nextItemKeys;
        // ... and reinitialize next
        delayLoadData.nextDeferred = $q.defer();
        delayLoadData.nextItemKeys = [];

        var searchTerm = '%' + delayLoadData.activeItemKeys.join(" %");
        modelItems.search(searchTerm, "", 0, 9998)
        .then(function () {
          activeDeferred.resolve();
          delayLoadData.activeItemKeys = [];

          // another flight waiting? Kick it off
          if (delayLoadData.nextItemKeys.length > 0) {
            delayLoad();
          }
        });
  ...
  //** User Related
  self.users = self.createModelCache(utilityHub.server.searchUsers);
  model.users = self.users;

  utilityHub.on('updateUsers', function (itemsData) {
    $rootScope.$apply($rootScope.$broadcast('updateUsers', self.updateItemsModel(self.users, itemsData)));
  });

  self.users.activeUsersIndexer = self.registerIndexer(self.users, {
    index: [],
  ...
  self.users.disabledUsersIndexer = self.registerIndexer(self.users, {
    index: [],
    sort: self.compareByProperties('id'),
    baseFilter: function (item) {
  ...
  /*
      model.users.activeUsersIndexer = {
        index: [],
        sort: self.compareByProperties('id'),
        baseFilter: function (item) {
          return item.state === USER_STATE.active;
  ...
  */
  ...
  self.searchConversations = function (searchExpression, sortExpression, startIndex, rowCount) {
    return callHub(function () {
  ...
  self.findConversation = function (conversationID) {
    return callHub(function () {
      return utilityHub.server.searchConversations("%" + conversationID, "", 0, 1);
    })
    .then(function (itemsData) {
      if (itemsData && itemsData.items && itemsData.items.length > 0) {
        return itemsData.items[0];
      }
      return null;
    });
  };

  self.getCustomerSupportConversation = function (customerUserID) {
    return callHub(function () {
      return utilityHub.server.getCustomerSupportConversation(customerUserID);
    })
    .then(function (itemsData) {
      if (itemsData && itemsData.items && itemsData.items.length > 0) {
        return itemsData.items[0];
  ...
  utilityHub.on('updateConversations', function (itemsData) {
    // rebroadcast out to any controllers that are listening
    $rootScope.$apply($rootScope.$broadcast('updateConversations', itemsData));
  ...
  // Updates a single conversation with data from itemsData
  self.updateConversation = function (conversation, itemsData) {
    if (!conversation) {
      $log.warn("utilityService.updateConversation(), no conversation");
      return;
    }
    ...
    if (angular.isArray(items)) {
      for (var i = 0; i < items.length; i++) {
        ...
        if (item && item.id === conversation.id) {
          mergeConversation(conversation, item);
          return;
        }
      }
    }
  ...
  function mergeConversation(conversation, item) {
    // ** Special handling for the messages - treat as a delta if we're only passed a handful of messages
    if (angular.isArray(conversation.messages) && item.messageCount && angular.isArray(item.messages) && item.messages.length < item.messageCount) {
      angular.forEach(item.messages, function (itemMessage) {
        // extend or append this message
        var existingConversationMessage = null;
        for (var j = 0; j < conversation.messages.length; j++) {
          var conversationMessage = conversation.messages[j];
          if (conversationMessage.userID === itemMessage.userID && conversationMessage.timestamp == itemMessage.timestamp) {
            existingConversationMessage = conversationMessage;
            break;
          }
        }

        if (existingConversationMessage) {
          angular.extend(existingConversationMessage, itemMessage);
        } else {
          conversation.messages.push(itemMessage);
        }
      });
    ...
    angular.extend(conversation, item);
  }
  ...
  var trackConversationsCount = 0;
  self.trackConversations = function () {
    if (++trackConversationsCount == 1) {
      joinGroup("conversations")
      .catch(function (failureData) {
        $log.error(failureData.errorMessage, failureData);
      })
      ;
    }
  };
  self.untrackConversations = function () {
    if (--trackConversationsCount == 0) {
  ...
  this.setConversationState = function (conversation, state) {
    return callHub(function () {
      return utilityHub.server.setConversationState(conversation.id, state);
    });
  };
...
*/
  ...
  self.setConversationLastReadTimestamp = function (conversation, timestamp) {
    return callHub(function () {
      return utilityHub.server.setConversationLastReadTimestamp(conversation.id, timestamp);
    });
  };


  self.setConversationRead = function(conversation) {

    var userID = msIdentity.userID;

    if (!conversation ||
      !conversation.customerUserID ||
      !angular.isArray(conversation.messages) ||
      !userID) {
      $log.error("utilityService.setConversationRead(), bailing", conversation);
    }
    // is caller the customer role (or support role)?
    var isCustomerRole = userID === conversation.customerUserID;

    for (var index = conversation.messages.length - 1; index >= 0; index--) {
      // is messsage a customer message (or support message)?
      var isCustomerMessage = conversation.messages[index].userID === conversation.customerUserID;
      // find the last message that isn't ours ...
      if (isCustomerRole != isCustomerMessage) {
        var timestamp = conversation.messages[index].timestamp;
        if (timestamp > conversation.customerLastReadTimestamp) {
          $log.debug("utilityService.setConversationRead(), message: ", conversation.messages[index]);
          return self.setConversationLastReadTimestamp(conversation, timestamp);
        }
        return;
      }
    }
  ...
  this.getDemoTenants = function () {
    return callHub(function () {
      return utilityHub.server.getDemoTenants();
    });
  };
  ...
  self.createDemoTenant = function (tenantName, data) {
    return callHub(function () {
      return utilityHub.server.createDemoTenant(tenantName, data);
    });
  };

  self.createDemoUser = function (tenantGroup, appRole) {
    return callHub(function () {
      return utilityHub.server.createDemoUser(tenantGroup.id, appRole);
    });
  };
  ...
  this.setUserPassword = function (user, newPassword) {
    return callHub(function () {
      return utilityHub.server.setUserPassword(user.id, newPassword);
    });
  };

  this.changeUserPassword = function (user, currentPassword, newPassword) {
    return callHub(function () {
      return utilityHub.server.changeUserPassword(user.id, currentPassword, newPassword);
    });
  };
  ...
  self.trackEventLog = function () {
    if (++trackEventLogCount == 1) {
      joinGroup("eventLog");
    }
  };
  self.untrackEventLog = function () {
  ...
  self.trackDatabaseLog = function () {
    if (++trackDatabaseLogCount == 1) {
      joinGroup("databaseLog");
    }
  };
  self.untrackDatabaseLog = function () {
  ...
  self.getActivityLogSet = function (itemID) {
    return callHub(function () {
  ...
  self.trackActivityLog = function () {
  ...
  self.untrackActivityLog = function () {
  ...
  self.updateItemsModel = function (itemsModel, itemsData) {
    ...
    if (angular.isFunction(itemsModel.unPackHandler)) {
      angular.forEach(itemsData.items || [], itemsModel.unPackHandler);
    }
  ...
  function callHub(hubRequest) {
    ...
    if (!hubReady) {
      // Shouldn't happen if we're well behaved. If we close the server connection (which results in hubReady being null), 
      // we should be at a safe state that doesn't need to query the server
    ...
    hubReady.done(function () {
      ...
      try {
        ...
        .fail(function (failResult) {
          $log.error("hub request failed", failResult);
          deferred.reject(failResult);
        });
      }
      catch (e) {
        // happens if say a hub method isn't defined
        $log.error("callHub(), exception: ", e);
        deferred.reject("Sorry, an unexpected error has occured (" + e.message + ")");
      }
    });
...
app.directive('msSearchView', function ($parse, $log, utilityService) {
  return {
    restrict: 'EA',
    ...
    controller: function ($scope, utilityService) {
      ...
      $scope.loadPage = function (searchExpression) {
        ...
        $log.debug("ms-search-view: ", $scope.loadPageSearchExpression);
...
app.filter('demandUser', ['utilityService', function (utilityService) {
  return function (objectID) {
    return utilityService.users.demand(objectID);
  };
}]);
